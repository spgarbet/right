    ...
    traj %>%
      branch(
        ...
        trajectory("not have") %>% # Use probability of ordering test
          branch(
            function() sample(1:2,1,prob=c(1- inputs$simvastatin$vProbabilityReactive,  inputs$simvastatin$vProbabilityReactive)),
              continue=c(TRUE,TRUE),
        ...
        branch(
          function() 
          {
            ...
            if(get_attribute(env, 'aGenotyped_CVD') != 1) return(1)
                   ...
                   (get_attribute(env, 'aOrdered_test') == 2 || runif(1) < inputs$simvastatin$vProbabilityRead) )

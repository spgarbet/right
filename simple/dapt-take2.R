...
# Define 1st primary treatment compartment
f_ta <- Vectorize(vectorize.args=c("t","a"), FUN=function(t, a, params)
{
  if(a < 0 || a > 1 || t-a <= 0) 0 else
    s(t) * w(t-a, params) * loss(a, params$f_loss)
})


# g_loss 1 and 2 is primary all, 3 is maintenance loss fatality only
# This helps in computing cross over from f -> m
params$g_loss    <- params$f_loss
params$g_loss[3] <- with(params, {phi_m[3] })


# Those that complete primary f without CABG and PCI, by t and a
gp_ta <- Vectorize(vectorize.args=c("t","a"), FUN=function(t, a, params)
{
  if(a <= 1 || t-a <= 0) 0 else
    s(t) * w(t-a, params) * loss(a, params$g_loss)
})
  
# Occupancy of primary treatment by time
f_t <- Vectorize(vectorize.args="t", function(t, params)
{
  integrate(function(a) f_ta(t, a, params), rel.tol=tolerance, lower=0, upper=pmin(t,1))$value
})

# CABG occurrance from primary compartment at time = t and of age = a
cp_ta <- Vectorize(vectorize.args=c("t","a"), FUN=function(t, a, params)
{
  ch <- t-a # characteristic line 
  if(a < 0 || ch <= 0) return(0)
  with(params, {
    if(a <= 30/365) return(
      s(t)*w(ch,params)*kappa_p[1]/(f_loss[1] - phi_m[1])*
        (exp(-phi_m[1]*a) - exp(-f_loss[1]*a))
    )
  
    # This is wrong, but right idea. Need better boundary conditions across model
    if(a <= 1) return({
      y1  <- cp_ta(t, 30/365, params)
    
      k   <- kappa_p[2]/(f_loss[2] - phi_m[2]) * w(ch, params) * 
             exp(-f_loss[2] * 30/365) 
      k   <- y1 + k
      k   <- k / exp(-phi_m[2] * 30/365)
  
      s(t)*(k*exp(-phi_m[2]*a) - kappa_p[2]*w(ch,params)*exp(-f_loss[2]*a)/(f_loss[2]-phi_m[2]))
    })
...
# Define 1st restart due to PCI compartment
# Losses from here are CABG, another PCI and death again
p1_ta <- Vectorize(vectorize.args=c("t","a"), FUN=function(t, a, params)
{
  if(a < 0 || a > 2 || t-a <= 0) 0 else
    s(t) * w(t-a,params) * (1-loss(min(a,1), params$pi_p)) * (loss(a, params$f_loss))
})

# Define death from primary
dp_ta <- Vectorize(vectorize.args=c("t","a"), FUN=function(t, a, params)
{
  if(a < 0 || a > 1 || t-a <= 0) 0 else
    s(t) * w(t-a,params) * (1-loss(a, params$phi_p)) * (loss(a, params$f_loss))
})

c1_ta <- Vectorize(vectorize.args=c("t","a"), FUN=function(t, a, params)
{
  p1_ta(t, a, params) * (1-loss(a, params$kappa_p))
})

m_ta <- function(t, a, params) 
{
  gp_ta(t, a, params) +  # Completion of primary therapy
  cp_ta(t, a, params) +  # CABG from primary
  c1_ta(t, a, params)    # CABG from PCI 1 group
}
...
x <- seq(0, 0.15, 0.003)
y <- x
z <- outer(x, y, function(a, b) cp_ta(a,b,params))

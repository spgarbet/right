...
days_till_in_range <- function(attrs, inputs)
{
  ...
  {
    if (PGx==1 & ava==1 & use==1)             {t2e <- rexp(1, inputs$warfarin$vMedianTimetoINR_PGx)} #genotype-guided dosing: no delay (test available at hand)
    else if (PGx==1 & ava==2 & use==1)        {t2e <- rexp(1, inputs$warfarin$vMedianTimetoINR_PGx_delay)} #genotype-guided dosing: with 3-day delay (reactive test this time) 

...
Clopidogrel <- function(t, y, params)
{
  with(as.list(c(y, params)), {
    ...
    cabg.r <- cabg.rates(params)
    
    # Fatal bleed loss

    # Starting 2nd phase of therapy after 1st month (minus mortality and cabg of those who started)
    start2   <- if (t < 30/365) 0 else lagderiv(t-30/365, 2)*exp(-F_drate(t,  30/365))*exp(-F_cabg.rate(30/365, 30/365, params))*exp(-fb*30/365)
    # Available at end of 12 months of therapy (phase1+phase2) minus mortality
    avail3   <- if (t < 1)      0 else lagderiv(t-335/365,3)*exp(-F_drate(t, 335/365))*exp(-F_cabg.rate(30/365, 30/365, params))*exp(-fb*335/365)
    ...
    finish2 <- if(t<=1) 0 else integrate(Vectorize(function(a){
          lagderiv(a, 4) * exp(-F_cabg.rate(a+1, 1, params))
        }),
        lower=max(0, t-2),
        upper=t-1
      )$value 
    ...
    list(c(
            ...
            phase1  = incoming - start2  - (fb+r_d+cabg.r[1])*phase1,
            phase2  = start2   - avail3  - (fb+r_d+cabg.r[2])*phase2,
            phase3  = start3   - finish2 - (r_d+cabg.r[3])*phase3,
            maint   = finish   + finish2 - r_d*maint+sum(cabg.r*c(phase1, phase2, phase3)),

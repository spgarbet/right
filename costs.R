...
cost.qaly = function(env,inputs) 
{
  ...
  arrivals$discounted_cost <- discount_value(value=arrivals$cost,A=arrivals$start_time,B=arrivals$end_time)
  ...
  qaly.i <- qaly2 %>%  mutate(disutility=ifelse(is.na(disutility),0,disutility)) %>% mutate(utility=pmin(1-disutility)) %>% mutate(time=lead(value)) %>% mutate(time=ifelse(row_number()>1,time-lag(time),time)) %>%  filter(!is.na(time)) %>% 

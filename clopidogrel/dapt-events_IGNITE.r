  ...
  traj %>%
    ...
    branch(
      function(attrs) {
        ...
        return(attrs[['aDAPT.Rx']]) # Default is to Clopidogrel, hence return 1 if no Hx of alternative drug, or if not switched.  
      },
      continue=rep(TRUE,3),
  ...
  traj %>%
    branch( 
      ...
      create_trajectory() %>%  
        ...
        ####
        # Switch at 30 day
        set_attribute("aDAPT30d",function(attrs) now(env) + dapt_30d(attrs,inputs)) %>%
...
##
####
dapt_30d <- function(attrs,inputs) {
  if (inputs$vSwitch!="None" & attrs[['aSwitch30d']]==1)
  {
    return(30)
  } else {
...
dapt_30d_strategy <- function(traj,inputs)
{
  ...
  if(inputs$vSwitch == "All") 
    ...
    traj %>%
      branch(
        ...
        continue=rep(TRUE,4),
        create_trajectory() %>% timeout(0),
        create_trajectory() %>% release("ticagrelor") %>% seize("clopidogrel") %>% set_attribute("aDAPT.Rx",1),
        create_trajectory() %>% release("prasugrel") %>% seize("clopidogrel") %>% set_attribute("aDAPT.Rx",1),
        create_trajectory() %>% timeout(0)
      )
    } else {
  #genotype at 30d
  traj %>% 
    ...
    branch(
      ...
      continue=rep(TRUE,2),
      create_trajectory("switch to Clopidogrel") %>%
        branch(
          function(attrs) ifelse(attrs[['aDAPT.Rx']] %in% c(1:3),attrs[['aDAPT.Rx']],4),
          continue=rep(TRUE,4),
          create_trajectory() %>% timeout(0),
